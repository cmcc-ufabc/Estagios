class AvisosController < ApplicationController
  ...
  # GET /avisos.json
    ...
    def realizou_matricula()
      ...
      unless @periodo.blank?
          ...
          @matriculas = Matricula.find(:all, :conditions => {:aluno_id => current_usuario.id, :periodo_id => @periodo.id, :status => [0,1,2,3]})
    ...
    unless @periodo.blank?
        ...
        if current_usuario.tipo == "Aluno"
            if realizou_matricula()
                ...
                @matriculas = Matricula.find(:all, :conditions => {:aluno_id => current_usuario.id, :periodo_id => Periodo.find(:last).id, :status => [0,1,2,3]})
            end
        end
  
    if current_usuario.centro == nil
      @avisos = Aviso.find(:all, :conditions => {:periodo_id => @periodo.id})
    else
      @avisos = Aviso.find(:all, :conditions => {:periodo_id => @periodo.id,:centro=>current_usuario.centro})
    end
    else
  ...
  def create
    ...
    @hoje = Date.current
    @aviso.data = @hoje
    @aviso.centro = current_usuario.centro
    respond_to do |format|
      if @aviso.save
