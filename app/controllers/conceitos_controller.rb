class ConceitosController < ApplicationController
  ...
  def dssi
    ...
    @ci=params[:ci].to_s
    @newci=@ci.slice(2,3)
    @matriculas = Matricula.find(:all, :conditions => {:periodo_id => params[:periodo], :numero_ci => @ci}, :order => "aluno_id")
  ...
  def show    
    ...
    unless @periodo.blank?
      ...
      @matriculas_aux = Matricula.find(:all, :conditions => {:periodo_id => params[:id], :status => 2})
